...
-- ----------------------------------------------------------------
-- TABLA: stable_poses
-- Posiciones estables de piezas determinadas por simulacion fisica.
-- Sustituye el concepto de stable_face en piece_embeddings.
-- Una fila por posicion estable detectada.
-- ----------------------------------------------------------------
CREATE TABLE IF NOT EXISTS stable_poses (
    ...
    set_id           TEXT,
    zenith_observable_area DOUBLE PRECISION,
    created_at       TIMESTAMPTZ DEFAULT NOW(),
    updated_at       TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE (part_ref, pose_index)
);

COMMENT ON COLUMN stable_poses.zenith_observable_area IS 'Área de la silueta de la pieza proyectada cenitalmente (desde arriba) sobre la cinta transportadora en mm2, basada en el Convex Hull 2D de la malla LDraw.';
