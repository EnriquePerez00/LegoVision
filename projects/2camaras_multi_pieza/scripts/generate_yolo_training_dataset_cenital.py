# -*- coding: utf-8 -*-
"""scripts/generate_yolo_training_dataset_cenital.py
===================================================
Genera el dataset de entrenamiento YOLO desde la cámara cenital
utilizando empaquetamiento 2D Monte Carlo.
"""
...
# ── Logging ──────────────────────────────────────────────────────────────────
import sys as _sys
_proj_log = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _proj_log not in _sys.path:
    _sys.path.insert(0, _proj_log)
from core.utils.logger import get_logger, log_execution_header, log_execution_footer
log = get_logger("yolo_cenital")
...
YOLO_TOTAL_PIECES_TARGET = getattr(cfg.yolo.dataset, "total_pieces_cenital", 1000)
...
def setup_camera():
    """Configura la cámara cenital simétrica."""
    target = bpy.data.objects.get("Camera_Target")
    if not target:
        bpy.ops.object.empty_add(type='PLAIN_AXES', location=(0.0, 0.0, 0.0))
        target = bpy.context.active_object
    ...
    for part in geom_list:
        ...
        if part_path:
            try:
                ...
                log.warning(f"Error importando {ref}: {e}")
        if not obj:
...
def pack_pieces_cenital(tmap, geom_list, color_palette, max_pieces=12):
    """Empaqueta piezas en el FOV cenital de 20x20 cm."""
    placed = []
    limit_bu = 9.5   # 20cm/2 - 0.5cm de margen
    margin_bu = 0.5  # 0.5cm separación mínima

    selected_parts = random.choices(geom_list, k=max_pieces * 3)
    attempts_total = 0
    max_attempts_total = 300

    for part in selected_parts:
        if len(placed) >= max_pieces or attempts_total >= max_attempts_total:
            break
        ...
        for _ in range(15):
            ...
            cx_min = -limit_bu + w_x/2.0
            cx_max = limit_bu - w_x/2.0
            cy_min = -limit_bu + h_y/2.0
            cy_max = limit_bu - h_y/2.0
            ...
            # Posicionar
            oc.location = (cx, cy, 0.0)
            bpy.context.view_layer.update()

            bbox_world = [oc.matrix_world @ mathutils.Vector(c) for c in oc.bound_box]
            x_min_new = min(pt.x for pt in bbox_world)
            x_max_new = max(pt.x for pt in bbox_world)
            y_min_new = min(pt.y for pt in bbox_world)
            y_max_new = max(pt.y for pt in bbox_world)

            # Validar colisiones
            overlap = False
            for p_obj, p_ref, old_bbox in placed:
                old_min_x, old_max_x, old_min_y, old_max_y = old_bbox
                if not (x_max_new + margin_bu < old_min_x or x_min_new - margin_bu > old_max_x or 
                        y_max_new + margin_bu < old_min_y or y_min_new - margin_bu > old_max_y):
                    overlap = True
                    break
            ...
            if not overlap:
                ...
                placed.append((oc, ref, (x_min_new, x_max_new, y_min_new, y_max_new)))
                break
            else:
                bpy.data.objects.remove(oc, do_unlink=True)
    return placed
...
def cleanup_placed_pieces():
    bpy.ops.object.select_all(action='DESELECT')
    keep = {"Conveyor_Belt_Plane", "Camera_Target", "Side_Rail_L", "Side_Rail_R", "Cam_Cenital", "Lab_Floor"}
    for o in list(bpy.context.scene.objects):
...
def main():
    ...
    log_execution_header(log, "generate_yolo_training_dataset_cenital.py",
                         output_dir=output_dir,
                         pieces_per_frame=f"{YOLO_PIECES_PER_FRAME_MIN}-{YOLO_PIECES_PER_FRAME_MAX}",
                         empty_frame_ratio=YOLO_EMPTY_FRAME_RATIO)
    ...
    log.info(f"Generando dataset YOLO Cenital | objetivo: {total_pieces_needed} piezas (config.yolo.dataset.total_pieces_cenital)")
    ...
    while total_pieces_placed < total_pieces_needed:
        ...
        if random.random() < YOLO_EMPTY_FRAME_RATIO:
            img_fn = f"train_cenital_{frame_idx:05d}.png"
            lbl_fn = f"train_cenital_{frame_idx:05d}.txt"
            ...
            log.debug(f"Frame vacío generado: cenital_{frame_idx:05d}")
            frame_idx += 1
            continue

        num_p = random.randint(YOLO_PIECES_PER_FRAME_MIN, YOLO_PIECES_PER_FRAME_MAX)
        placed = pack_pieces_cenital(tmap, geom_list, color_palette, max_pieces=num_p)
        ...
        if labels:
            ...
            try:
                ...
                log.warning(f"Render fallido frame {frame_idx}: {e}")
                continue
            ...
            log.info(f"Frame cenital {frame_idx+1:04d} | piezas={len(labels):2d} | total={total_pieces_placed}/{total_pieces_needed}")
            frame_idx += 1
        else:
            log.debug(f"Frame {frame_idx+1} descartado: ninguna pieza en FOV")

    cleanup_placed_pieces()
    for obj in list(tmap.values()):
        bpy.data.objects.remove(obj)
    _duration = _time.perf_counter() - _t_start
    log_execution_footer(log, "generate_yolo_training_dataset_cenital.py",
                         duration_s=_duration,
                         total_frames=frame_idx,
                         total_piezas=total_pieces_placed,
                         output_dir=output_dir)
