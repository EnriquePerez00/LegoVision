...
def main():
    if not IN_BLENDER:
        # Multiprocessing Manager: Intercept command-line call to run Blender instances in parallel
        ...
        parser.add_argument("--all", action="store_true", help="Simulate all unique parts in set_catalog")
        ...
        if args.part:
            print("[ERROR] Para simular una sola parte específica, ejecuta el script desde Blender.", flush=True)
            return
            
        # Obtener lista de piezas del catálogo
        from core.db.set_catalog import REAL_SETS
        if args.all:
            parts = []
            for sid, set_data in REAL_SETS.items():
        ...
        target_name = "todas las piezas del catálogo" if args.all else f"el set {args.set_id}"
        print(f"[SimStablePoses Manager] Iniciando simulación paralela para {len(parts)} piezas de {target_name}...", flush=True)
        max_workers = max(1, os.cpu_count() - 2)
        print(f"[SimStablePoses Manager] CPU Cores: {os.cpu_count()}, Workers activos: {max_workers}", flush=True)
        
        # Encontrar la ruta de Blender en macOS
        blender_path = "/Applications/Blender.app/Contents/MacOS/Blender"
        if not os.path.exists(blender_path):
            blender_path = "blender" # Intentar desde el PATH
            
        def run_single_part(part_ref):
            print(f"[Worker] Empezando simulación física en Blender para pieza: {part_ref}", flush=True)
            # Re-invocarse a sí mismo pasándole Blender y la pieza específica
            cmd = [
                blender_path, "--background", "--python", __file__, 
                "--", "--part", part_ref, "--set_id", args.set_id, "--n_dirs", str(args.n_dirs)
            ]
            if args.save_db:
                cmd.append("--save_db")
            res = subprocess.run(cmd, capture_output=True, text=True)
            if res.returncode == 0:
                print(f"[Worker] ✓ Simulación completada con éxito para pieza: {part_ref}", flush=True)
            else:
                print(f"[Worker] ✗ Error en simulación para pieza {part_ref}. Return code: {res.returncode}", flush=True)
                print(res.stderr, flush=True)
            return part_ref, res.returncode
            
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            results = list(executor.map(run_single_part, sorted(parts)))
            
        print("\n[SimStablePoses Manager] Todas las simulaciones paralelas han finalizado.", flush=True)
        return

    # Blender-level execution logic
    args_list = []
    if "--" in sys.argv:
    ...
    print("[SimStablePoses] " + str(len(parts)) + " piezas a simular")
    results = []
