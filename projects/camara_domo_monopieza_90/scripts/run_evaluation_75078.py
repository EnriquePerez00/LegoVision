...
def run_evaluation(metadata_path, report_path, use_dinov2_color=False, use_emd_color=False, color_classifier_name="75078-1"):
    ...
    log.info("Cargando detector YOLO cenital (GPU/MPS)...")
    yolo_cen_path = os.path.join(project_root, "models", "yolo_cenital.pt")
    yolo_cen = YOLO(yolo_cen_path).to(device)
    yolo_lat = None
    yolo_cen_pose = None
    yolo_lat_pose = None

    if USE_LATERAL_CAMERA:
        log.info("Cargando detector YOLO lateral...")
    ...
    # Pre-calcular predicciones YOLO por lotes (Batching) al inicio en GPU/MPS
    cen_predictions = {}
    lat_predictions = {}
    
    unique_cen_paths = set()
    unique_lat_paths = set()
    for r in renders:
        if "cameras" in r:
            if "cenital" in r["cameras"] and r["cameras"]["cenital"].get("image_path"):
                unique_cen_paths.add(r["cameras"]["cenital"]["image_path"])
            if "lateral" in r["cameras"] and r["cameras"]["lateral"].get("image_path"):
                unique_lat_paths.add(r["cameras"]["lateral"]["image_path"])
        for obs in r.get("observations", []):
            unique_cen_paths.add(obs["frame_img_path"])
            img_lat_path = obs["frame_img_path"].replace("_cenital.png", "_lateral.png") if "_cenital" in obs["frame_img_path"] else obs["frame_img_path"].replace(".png", "_frontal.png")
            unique_lat_paths.add(img_lat_path)

    # Inferencia por lotes Cenital
    unique_cen_list = sorted(list(unique_cen_paths))
    log.info(f"[YOLO Batch] Pre-procesando {len(unique_cen_list)} imágenes cenitales en {device}...")
    batch_size = 16
    for i in range(0, len(unique_cen_list), batch_size):
        batch = unique_cen_list[i:i+batch_size]
        try:
            yolo_results = yolo_cen(batch, conf=yolo_conf, imgsz=yolo_imgsz, verbose=False, batch=len(batch))
            for path, res in zip(batch, yolo_results):
                cen_predictions[path] = res.boxes
        except Exception as e:
            log.error(f"[YOLO Batch Error] Error procesando lote cenital: {e}")

    # Inferencia por lotes Lateral
    if USE_LATERAL_CAMERA and yolo_lat is not None:
        unique_lat_list = sorted(list(unique_lat_paths))
        log.info(f"[YOLO Batch] Pre-procesando {len(unique_lat_list)} imágenes laterales en {device}...")
        for i in range(0, len(unique_lat_list), batch_size):
            batch = unique_lat_list[i:i+batch_size]
            try:
                yolo_results = yolo_lat(batch, conf=0.15, imgsz=1024, verbose=False, batch=len(batch))
                for path, res in zip(batch, yolo_results):
                    lat_predictions[path] = res.boxes
            except Exception as e:
                log.error(f"[YOLO Batch Error] Error procesando lote lateral: {e}")
    ...
    def process_entry(entry):
        ...
        # Obtener cajas de YOLO precalculadas en batch
        yolo_boxes_cen = cen_predictions.get(img_cen_path)
        if yolo_boxes_cen is None or len(yolo_boxes_cen) == 0:
            with lock:
                log.warning(f"[{sample_idx+1:02d}] No se detectó pieza en cámara cenital (precalculado).")
                iou_list.append(0.0)
            return
        ...
        if USE_LATERAL_CAMERA and img_lat is not None:
            ...
            yolo_boxes_lat = lat_predictions.get(img_lat_path) if (yolo_lat is not None and img_lat_path in lat_predictions) else None
            ...
            if entry.get("is_simulation") and best_height_obs.get("bbox_frontal_norm"):
                ...
                box_lat = True
            elif yolo_boxes_lat is not None and len(yolo_boxes_lat) > 0:
                box_lat = yolo_boxes_lat[0]
        ...
        # --- 4. Clasificación Neuro-Simbólica Final ---
        with gpu_lock:
