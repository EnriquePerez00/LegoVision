...
output_sim_dir = os.path.join(project_root, "data", "simulation_x5_1D_all")
...
def find_blender():
    candidates = [
        ...
        "/Users/I764690/Applications/Blender.app/Contents/MacOS/Blender",
...
def main():
    ...
    parser.add_argument("--num_pieces", type=int, default=895, help="Número de piezas a simular para garantizar cobertura del catálogo.")
    parser.add_argument("--frames_per_piece", type=int, default=1, help="Frames por pieza para tracking rápido.")
    parser.add_argument("--resolution", type=int, default=1024, help="Resolución de los renders.")
    args = parser.parse_args()

    num_pieces = args.num_pieces
    frames_per_piece = args.frames_per_piece
    resolution = args.resolution

    has_renders = os.path.exists(metadata_path) and len([f for f in os.listdir(output_sim_dir) if f.endswith('.png')]) >= 120
    if has_renders:
        print(f"--- 1. Renders y metadatos encontrados en {output_sim_dir}. Omitiendo simulación. ---")
    else:
        print(f"--- 1. Generando metadatos para la simulación de {num_pieces} piezas a {resolution}x{resolution} ---")
        ...
        # 1.2 Ejecutar metadata_only para empaquetado
        cmd_meta = [
            blender_path, "-b", "-P", sim_script, "--",
            "--num_pieces", str(num_pieces),
            "--output_dir", output_sim_dir,
            "--speed", "0.083333",
            "--frames_per_piece", str(frames_per_piece),
            "--set-id", "x5_all",
            "--dimension", "1D",
        ...
        for worker_id in range(num_workers):
            ...
            log_file = open(os.path.join(project_root, "logs", f"sim_worker_200_all_{worker_id}.log"), "w")
    ...
    print("\n--- 2. Ejecutando inferencia neuronal con pipeline 1D_all ---")
    eval_script = os.path.join(project_root, "scripts", "run_evaluation_1D_all.py")
    cmd_eval = [
...
def evaluate_results():
    ...
    print(f"--- REPORTE DE EXACTITUD (simulation_x5_1D_all: {N} piezas) ---")
    print(f"Exactitud de Referencia (Geometría): {ref_correct/N*100:.2f}%")
    ...
    print("\n--- Análisis de Fallos de Color (Fusionado) ---")
    fallos = []
