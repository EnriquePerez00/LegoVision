...
class LegoEfficientNetClassifierAll:
    """Neuro-symbolic classifier for Lego piece geometry using EfficientNetV2-B0 and database filters for all parts."""
    ...
    def __init__(self):
        ...
        # Inicializar catálogo e info para todas las piezas
        self.allowed_refs = {k[0] for k in self.pose_face_class.keys()}
        if not self.allowed_refs:
            self.allowed_refs = set()
            for s_id, s_data in REAL_SETS.items():
                for p in s_data.get("parts", []):
                    self.allowed_refs.add(p["ref"])
        ...
        try:
            with supabase_client.get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT part_ref, color_name FROM lego_set_parts;")
                    rows = cur.fetchall()
                    for row in rows:
                        if row["color_name"]:
                            self.part_to_colors.setdefault(row["part_ref"], set()).add(row["color_name"].strip().lower())
            print(f"[All Pieces Classifier] Loaded {len(self.part_to_colors)} part-color mappings from Database.")
        except Exception as e:
            print(f"[All Pieces Classifier Warning] Could not load color mappings from DB: {e}")
            
        for s_id, s_data in REAL_SETS.items():
            for p in s_data.get("parts", []):
                if p["ref"] not in self.part_to_colors:
                    if p.get("color_name"):
                        self.part_to_colors.setdefault(p["ref"], set()).add(p["color_name"].strip().lower())
                self.part_qtys[p["ref"]] = self.part_qtys.get(p["ref"], 0) + p["qty"]
        
        total_qty = sum(self.part_qtys.values()) if self.part_qtys else 1
        self.part_priors = {ref: qty / total_qty for ref, qty in self.part_qtys.items()}
        self.color_name_to_code = {}
        try:
            palette_path = os.path.join(project_root_local, "data", "color_calibration_palette.json")
            if os.path.exists(palette_path):
                with open(palette_path, "r", encoding="utf-8") as f:
                    ...
                    for item in palette:
                        ...
                        if name and code:
                            self.color_name_to_code[name] = str(code)
            print(f"[All Pieces Classifier] Loaded {len(self.color_name_to_code)} colors in lookup.")
        except Exception as e:
            print(f"[All Pieces Classifier Warning] Could not load color palette for lookup: {e}")
            
        for s_id, s_data in REAL_SETS.items():
            for p in s_data.get("parts", []):
                if p["color_name"] not in self.color_name_to_code:
                    self.color_name_to_code[p["color_name"]] = str(p["color_code"])
        print(f"[All Pieces Classifier] Restricting universe to {len(self.allowed_refs)} parts.")

        # Precalentamiento del caché de embeddings de referencias para evitar consultas a la DB por pieza
        print("[Features Classifier] Precalentando caché de embeddings para todas las referencias...", flush=True)
        try:
            # Dado que el universo 'all' puede tener muchas piezas, cargamos de forma dinámica o precalentamos una parte.
            # Para evitar sobrecargar la memoria/tiempo si hay miles, filtramos solo por las permitidas que tengan pose estable.
    ...
    def classify(self, 
                 ...
                 detected_studs: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        # Mecanismo 1: Tolerancia de Área Dinámica según Tamaño de Pieza
        pruned_candidates = []
        for ref in candidates:
            ...
            # Aspect ratio hard pruning
            ratio_pruned = False
            if measured_length and measured_width and measured_length > 0.0 and measured_width > 0.0:
                ratio_query = max(measured_length, measured_width) / (min(measured_length, measured_width) + 1e-8)
                has_nominal_ratio = any(p.get("contact_stable_length") is not None and p.get("contact_stable_width") is not None for p in poses)
                if has_nominal_ratio:
                    any_ratio_match = False
                    for p in poses:
                        if p.get("is_stable", True):
                            nom_l = p.get("contact_stable_length")
                            nom_w = p.get("contact_stable_width")
                            if nom_l and nom_w and nom_l > 0.0 and nom_w > 0.0:
                                ratio_nom = max(nom_l, nom_w) / (min(nom_l, nom_w) + 1e-8)
                                rel_ratio_err = abs(ratio_query - ratio_nom) / ratio_nom
                                if rel_ratio_err <= 0.20:
                                    any_ratio_match = True
                                    break
                    if not any_ratio_match:
                        ratio_pruned = True

            if not area_pruned and not ratio_pruned:
                pruned_candidates.append(ref)
        ...
        if crop_cen is not None:
            ar_query = max(crop_cen.width, crop_cen.height) / (min(crop_cen.width, crop_cen.height) + 1e-8)
            lookup = self.ref_metadata.get("lookup", {})
            for ref in candidates:
                ref_ar_list = []
                prefix = f"ref_{ref}_"
                for fname, bbox_dict in lookup.items():
                    if fname.startswith(prefix) and "cenital" in bbox_dict:
                        x1, y1, x2, y2 = bbox_dict["cenital"]
                        w_ref = x2 - x1
                        h_ref = y2 - y1
                        ar_ref = max(w_ref, h_ref) / (min(w_ref, h_ref) + 1e-8)
                        ref_ar_list.append(ar_ref)
                
                if ref_ar_list:
                    ar_ok = False
                    for ar_ref in ref_ar_list:
                        if 0.80 * ar_ref <= ar_query <= 1.20 * ar_ref:
                            ar_ok = True
                            break
                    if not ar_ok:
                        gating_multipliers[ref] *= 0.5  # Penalización de relación de aspecto suave
            ...
            for ref in candidates:
                ...
                for pose in poses:
                    if pose.get("is_stable", True):
                        ...
                        if nom_l and nom_w:
                            if (nom_l * 0.60 <= measured_length <= nom_l * 1.50) and (nom_w * 0.60 <= measured_width <= nom_w * 1.50):
                                dim_ok = True
                                break
        ...
        for (part_ref, pose_index), refs in class_groups.items():
            ...
            if query_emb_lat is not None and lat_refs:
                # Decoupled Multi-View Fusion: 70% Cenital, 30% Lateral (rotation-immune)
