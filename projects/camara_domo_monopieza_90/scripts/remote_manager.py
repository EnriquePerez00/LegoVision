...
def check_kaggle(active_acc):
    ...
    try:
        ...
        status_obj = getattr(status_res, "status", "unknown")
        status = str(status_obj).lower()
        print(f"[Remote Manager Domo] Estado del Kernel en Kaggle: {status_obj}")
        ...
        elif any(x in status for x in ["error", "fail", "cancel"]):
            print(f"[Remote Manager Domo] El Kernel ha fallado con estado: {status_obj}")
