...
def main():
    ...
    # Realizar sharding si hay múltiples workers
    if pa.num_workers > 1:
        chunk_size = math.ceil(len(selected_parts) / pa.num_workers)
        start_idx = pa.worker_id * chunk_size
        end_idx = min(start_idx + chunk_size, len(selected_parts))
        my_parts = selected_parts[start_idx:end_idx]
        print(f"[Worker {pa.worker_id}/{pa.num_workers}] Procesará {len(my_parts)} piezas (índices {start_idx} a {end_idx - 1}).")
    else:
        my_parts = selected_parts
    ...
    for idx, ref in enumerate(my_parts):
        global_idx = (pa.worker_id * math.ceil(len(selected_parts) / pa.num_workers) + idx) if pa.num_workers > 1 else idx
        print(f"\n[Piece {global_idx+1}/{pa.num_pieces}] Procesando pieza {ref}...")
