# -*- coding: utf-8 -*-
"""run_simulation_100_75078.py
================================
Genera 100 piezas del set 75078-1, una detrás de otra, renderizadas
a 2048×2048 y las almacena en `data/simulation_100_75078`.
Luego ejecuta el pipeline de inferencia `run_evaluation_75078.py`
y reporta la exactitud consolidada.

Basado en `run_simulation_100_all.py` pero restringido al set 75078-1.
"""
import os
import sys
import subprocess
...
output_sim_dir = os.path.join(project_root, "data", "simulation_100_75078-1_1D")
...
def main():
    ...
    if has_renders:
        ...
        print(f"--- 1. Generando metadatos para la simulación de 100 piezas del set {SET_ID} a {resolution}x{resolution} ---")
        ...
        for worker_id in range(num_workers):
            ...
            log_file = open(
                os.path.join(project_root, "logs", f"sim_worker_100_75078_{worker_id}.log"),
                "w"
            )
        ...
        print(f"--- Renders completados para 100 piezas del set {SET_ID} a {resolution}x{resolution} ---")

    print("\n--- 2. Ejecutando inferencia neuronal con pipeline run_evaluation_75078 ---")
...
def evaluate_results():
    ...
    from scripts.run_evaluation_75078 import LEGO_TO_REBRICKABLE
    ...
    print(f"--- REPORTE DE EXACTITUD (simulation_100_75078: {N} piezas @ {resolution}x{resolution}) ---")
