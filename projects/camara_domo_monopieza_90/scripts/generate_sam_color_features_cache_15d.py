#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""generate_sam_color_features_cache_15d.py
Genera mlp_features_cache.npz con características reales de 15 dimensiones.
"""
import json
import os
import sys
...
def extract_features_sam_15d(img_path, bbox_norm, sam_model, min_pixels=10):
    ...
    diff_L = ml[0] - belt_median_lab[0]
    diff_a = ml[1] - belt_median_lab[1]
    diff_b = ml[2] - belt_median_lab[2]

    return np.array([ml[0],sl[0],ml[1],sl[1],ml[2],sl[2],
                     mh[0],sh[0],mh[1],sh[1],mh[2],sh[2],
                     L_p95, tex_contrast, tex_homogeneity,
                     diff_L, diff_a, diff_b, L_p5], dtype=np.float32)

def main():
    META_PATH = os.path.join(_ROOT,"data","simulation_1000_database_2D","simulation_metadata.json")
    DATA_DIR  = os.path.join(_ROOT,"data","simulation_1000_database_2D")
    CACHE_OUT = os.path.join(_ROOT,"data","mlp_features_cache.npz")
    SAM_MODEL = "mobile_sam.pt"

    dev = "mps" if torch.backends.mps.is_available() else "cpu"
    print("[generate_sam_cache_15d] Device:", dev)
    ...
    for frame in meta["frames"]:
        ...
        for p in frame.get("visible_pieces",[]):
            ...
            seen_positions[key].append({
                ...
                "color_code": str(p["color_code"]),
                "x_mm": p["x_belt_local_mm"]
            })
            
    for key, obs_list in seen_positions.items():
        best_obs = min(obs_list, key=lambda x: abs(x["x_mm"]))
        samples.append(best_obs)
    total = len(samples)
    ...
    for i,s in enumerate(samples):
        ...
        feat = extract_features_sam_15d(s["img_cen"], s["bbox"], sam)
    ...
    print("Cache guardado en:", CACHE_OUT)
