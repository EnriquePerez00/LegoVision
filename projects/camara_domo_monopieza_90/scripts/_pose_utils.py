...
def select_pose_tarps(
    ...
    cylindrical_parts = {"61184", "32054", "2780", "6558", "4274"}

    for p in poses:
        if p.get("is_stable") is False:
            continue

        normal = p.get("contact_normal")
        w_mm = p.get("contact_stable_width")
        
        # Filtro preventivo para orientaciones cilíndricas verticales (eje Y en LDraw)
        if part_ref in cylindrical_parts and normal and len(normal) == 3:
            if abs(normal[1]) > 0.85:
                continue

        if normal and len(normal) == 3 and w_mm is not None:
            # Filtro adicional para piezas cilíndricas y conectores
            if part_ref in cylindrical_parts:
                max_comp = max(abs(normal[0]), abs(normal[1]), abs(normal[2]))
                if max_comp < 0.95:
                    continue
                if (p.get("tipping_energy_ratio") or 0.0) < 0.05:
                    continue
