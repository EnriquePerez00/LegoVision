...
from scripts.rotation_aligner import align_image_by_moments


def load_efficientnet(device, weights_path=None):
    try:
        import timm
        if weights_path and os.path.exists(weights_path):
            ...
            print("Loaded pretrained ImageNet model weights")
    except Exception as e:
        print(f"Error loading model, fallback to torchvision: {e}")
...
def _parse_and_load_cropped(p, regex, transform, metadata_lookup, cam_name):
    ...
    try:
        ...
        if fname in metadata_lookup:
            ...
            aligned = align_image_by_moments(crop_img)
            img_proc = preprocess_crop_grayscale(aligned)
        else:
            aligned = align_image_by_moments(img)
            img_proc = preprocess_crop_grayscale(aligned)
...
def main():
    ...
    possible_dirs = [
        os.path.join(legovic_root, "projects", "2camaras_random_pieza_unica", "data", "dinov2_refs_v4_canonical"),
        os.path.join(legovic_root, "projects", "2camaras_random_pieza_unica", "data", "dinov2_refs_v3_canonical"),
        os.path.join(legovic_root, "projects", "2camaras_random_pieza_unica", "data", "dinov2_refs"),
    ...
    device = get_device()
    weights_cen = os.path.join(project_root, "models", "efficientnet_cenital.pt")
    weights_lat = os.path.join(project_root, "models", "efficientnet_lateral.pt")
    model_cen = load_efficientnet(device, weights_cen)
    model_lat = load_efficientnet(device, weights_lat)
    ...
    if os.path.isdir(cenital_dir):
        ...
        n, f = index_directory_cropped(paths, regex, model_cen, transform, device, face_id=1, metadata_lookup=metadata_lookup, cam_name="cenital")
        total_indexed += n
        total_failed += f
    ...
    if os.path.isdir(lateral_dir):
        ...
        n, f = index_directory_cropped(paths, regex, model_lat, transform, device, face_id=2, metadata_lookup=metadata_lookup, cam_name="lateral")
        total_indexed += n
        total_failed += f
