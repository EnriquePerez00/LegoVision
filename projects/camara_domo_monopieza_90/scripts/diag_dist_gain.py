...
from color_classifier_all import ColorClassifierAll, rgb_to_lab
...
def weighted_delta_e(lab1, lab2, w_L=1.0):
    dL = lab1[0] - lab2[0]
    da = lab1[1] - lab2[1]
    db = lab1[2] - lab2[2]
    return math.sqrt(w_L * (dL**2) + da**2 + db**2)

# Distancia a catalog colors con factor de ganancia de simulación de 1.56
gain = 1.56
for c in hierarchical_clf.catalog_colors:
    if c["color_name"] in ["Dark Bluish Gray", "Dark Azure", "White", "Black", "Light Bluish Gray"]:
        ...
        # Aplicar ganancia de iluminación de simulación
        rgb_ref_sim = np.clip(rgb_ref_nominal * gain, 0, 255)
        
        lab_ref = rgb_to_lab(rgb_ref_sim)
        
        dist_ciede = weighted_delta_e(lab_est, lab_ref, w_L=1.0)
        print(f"Catalog {c['color_name']:20s} | Nominal: {rgb_ref_nominal.tolist()} | Sim: {rgb_ref_sim.tolist()} | LAB Sim: {lab_ref} | CIELAB Dist: {dist_ciede:.4f}")
