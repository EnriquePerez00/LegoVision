...
def local_rgb_to_hsv(rgb):
    ...
    v = mx*255.0
    h = h / 2.0  # Scale to match OpenCV [0-180]
...
def augment_dataset(X, y_names, camera_name):
    ...
    color_db = {}
    gain = 1.56  # Blender Eevee light gain
    for item in palette:
        name = item["color_name"].strip().lower()
        hex_str = item["color_hex"].lstrip("#")
        rgb = np.array([int(hex_str[i:i+2], 16) for i in (0, 2, 4)], dtype=np.float32)
    ...
    for target_name, target_info in color_db.items():
        src_color = None
        # Try to find a seed color from the same group
        ...
        for _ in range(samples_per_class):
            ...
            # Shading and illumination jitter
            shadow_factor = random.uniform(0.70, 1.15)
            feat[0] *= shadow_factor
            feat[10] *= shadow_factor
            

            
            feat[6] = (feat[6] + random.uniform(-4.0, 4.0)) % 180.0
            feat[8] = np.clip(feat[8] + random.uniform(-10.0, 10.0), 0.0, 255.0)
            feat[10] = np.clip(feat[10] + random.uniform(-20.0, 10.0), 0.0, 255.0)
            
            feat[1] = np.clip(feat[1] * random.uniform(0.85, 1.25), 1.0, 40.0)
            feat[3] = np.clip(feat[3] * random.uniform(0.85, 1.25), 1.0, 40.0)
            feat[5] = np.clip(feat[5] * random.uniform(0.85, 1.25), 1.0, 40.0)
            feat[7] = np.clip(feat[7] * random.uniform(0.85, 1.25), 1.0, 40.0)
            feat[9] = np.clip(feat[9] * random.uniform(0.85, 1.25), 1.0, 40.0)
            feat[11] = np.clip(feat[11] * random.uniform(0.85, 1.25), 1.0, 40.0)
...
def main():
    ...
    print("Generating augmented datasets in parallel (2 processes)...")
    ...
    print(f"Total synthetic samples generated: {len(X_all)}")
    ...
    model_path = os.path.join(MODELS_DIR, "color_all.pt")
    ...
    metadata_path = os.path.join(MODELS_DIR, "color_all_metadata.json")
