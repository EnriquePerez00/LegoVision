...
for idx, frame in enumerate(meta_data["frames"][:97]):
    ...
    if len(pixels_rgb) > 0:
        ...
        color_rgbs.setdefault(c_name, []).append(median_rgb)

print("Empirical Simulation Colors mapping:")
mapping = {}
for c_name, rgbs in color_rgbs.items():
    mean_rgb = np.mean(rgbs, axis=0).round(1)
    mapping[c_name] = mean_rgb.tolist()
    print(f"'{c_name}': {mean_rgb.tolist()},")
