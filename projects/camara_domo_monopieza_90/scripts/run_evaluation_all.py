...
from scripts.efficientnet_classifier_all import LegoEfficientNetClassifierAll
...
def estimate_color_mlp_features(img, mask_binary, camera_type=None, ccm_params=None, is_simulation=False):
    """Extrae las 12 características estadísticas (Lab/HSV) robustas para el clasificador de color."""
    ...
    # Aplicar calibración CCM inversa si está configurada (solo en entorno real)
    ...
    # Filtrado IQR de outliers robusto en canales L, a, b (eliminar sombras/especulares)
    ...
    # Usar Mediana robusta en vez de la Media para representar el color difuso plano
...
# ── Pipeline Principal ──
def run_evaluation(metadata_path, report_path, use_dinov2_color=False, color_classifier_name="75078-1"):
    import os
    ...
    if color_classifier_name == "75078-1":
        try:
            from color_classifier_75078_1 import ColorClassifier75078_1
            hierarchical_clf = ColorClassifier75078_1(device=device)
        except Exception as e:
            log.error(f"Error cargando clasificador de color 75078-1: {e}")
    elif color_classifier_name == "all_colors":
        try:
            from color_classifier_all import ColorClassifierAll
            hierarchical_clf = ColorClassifierAll(device=device)
        except Exception as e:
            log.error(f"Error cargando clasificador de color all: {e}")


    ccm_path = os.path.join(project_root, "data", "ccm_dome_light.json")
    ccm_params = None
    is_simulation = "frames" in meta_data
    if os.path.exists(ccm_path) and not is_simulation:
        try:
        ...
        log.info("Omitiendo calibración de color CCM para simulación sintética.")
    ...
    def process_entry(entry):
        ...
        # Enfoque 1: Filtrar posturas inestables verticales de pines cilíndricos (61184 y 32054)
        if ref_gt in ["61184", "32054"]:
            area_gt = entry.get("zenith_silhouette_area_gt")
            if area_gt is not None and area_gt < 50.0:
                with lock:
                    log.info(f"Ignorando muestra {sample_idx} ({ref_gt}): pose vertical inestable (Enfoque 1)")
                return
        ...
        color_name_fused = "Unknown"
        color_code_fused = "0"
        color_hex_fused = "#808080"
        detected_colors = []
        ...
        if hierarchical_clf is not None:
            for obs in obs_to_eval:
                ...
                if img_c is not None:
                    ...
                    feat_cen = estimate_color_mlp_features(img_c, mask_bin, "cenital", ccm_params, is_simulation=is_simulation)
                    if feat_cen is not None:
                        p_cen = hierarchical_clf.predict_cenital_probs(feat_cen)
                        if np.sum(p_cen) > 0:
                ...
                if USE_LATERAL_CAMERA:
                    lat_path = None
                    bbox_lat_n = None
                    if "cameras" in entry and "lateral" in entry["cameras"]:
                        lat_path = entry["cameras"]["lateral"]["image_path"]
                        bbox_lat_n = entry["cameras"]["lateral"]["bbox_norm"]
                    elif "frame_img_path_lateral" in obs:
                        lat_path = obs["frame_img_path_lateral"]
                        bbox_lat_n = obs.get("bbox_frontal_norm")
                    elif cen_path:
                        lat_path = cen_path.replace(".png", "_frontal.png")
                        bbox_lat_n = obs.get("bbox_frontal_norm")

                    if lat_path and os.path.exists(lat_path) and bbox_lat_n:
                        img_l = _get_img(lat_path)
                        if img_l is not None:
                            w_l, h_l = img_l.size
                            lx1_n, ly1_n, lx2_n, ly2_n = bbox_lat_n
                            lpx1 = max(0, int(lx1_n * w_l)); lpy1 = max(0, int(ly1_n * h_l))
                            lpx2 = min(w_l, int(lx2_n * w_l)); lpy2 = min(h_l, int(ly2_n * h_l))
                            mask_bin_lat = _get_sam_mask(img_l, [lpx1, lpy1, lpx2, lpy2], (h_l, w_l))

                            feat_lat = estimate_color_mlp_features(img_l, mask_bin_lat, "lateral", ccm_params, is_simulation=is_simulation)
                            if feat_lat is not None:
                                p_lat = hierarchical_clf.predict_lateral_probs(feat_lat)
                                if np.sum(p_lat) > 0:
                                    conf_lat = float(np.max(p_lat))
                                    if p_lat_sum is None:
                                        p_lat_sum = np.zeros_like(p_lat)
                                    p_lat_sum += conf_lat * p_lat
                                    w_lat_sum += conf_lat

        # Promediar probabilidades acumuladas
        p_cen_agg = p_cen_sum / w_cen_sum if (w_cen_sum > 0 and p_cen_sum is not None) else None
        p_lat_agg = p_lat_sum / w_lat_sum if (w_lat_sum > 0 and p_lat_sum is not None) else None

        if p_cen_agg is not None and p_lat_agg is not None:
            p_combined = p_cen_agg * p_lat_agg
            if np.sum(p_combined) == 0:
                p_combined = p_cen_agg + p_lat_agg
        elif p_cen_agg is not None:
            p_combined = p_cen_agg
        elif p_lat_agg is not None:
            p_combined = p_lat_agg
        else:
            p_combined = None
        ...
        if p_combined is not None and len(p_combined) > 0 and associated_set and associated_set != "random":
            try:
                ...
                if associated_set in REAL_SETS:
                  ...
                  # Color resolution deferred to Phase 4 (3-Pass Gated Color Classification)
            except Exception as e:
        ...
        with gpu_lock:
            yolo_res_cen = yolo_cen(img_cen, verbose=False, conf=0.25, imgsz=1024)
        ...
        if USE_LATERAL_CAMERA and img_lat is not None:
            ...
            if box_lat is not None:
                ...
                if sam_res_lat and sam_res_lat[0].masks is not None and len(sam_res_lat[0].masks.data) > 0:
                    ...
                    raw_measured_height = measure_lateral_height_mm_sam(mask_lat, img_res_px_val=w_l)
                    
                    x_belt_h = best_height_obs.get("x_belt_local_mm", 0.0)
                    y_belt_h = best_height_obs.get("y_belt_local_mm", 0.0)
                    D_center = 320.0
                    D_piece = np.sqrt((x_belt_h - 320.0)**2 + y_belt_h**2)
        ...
        # --- 4. Clasificación Neuro-Simbólica Final (3-Pass Gated Color Pipeline) ---
        allowed_color_names = set()
        with gpu_lock:
            # Pass 1: Clasificación de geometría preliminar (sin color)
            preds_first_pass = efficientnet_clf.classify(
                crop_cen=crop_cen,
                mask_cen=mask_cen,
                crop_lat=crop_lat,
                mask_lat=mask_lat,
                ...
                detected_color=None,
                ref_gt=ref_gt,
                measured_height=measured_height if not entry.get("is_simulation", False) else 0.0,
                measured_length=measured_length,
                measured_width=measured_width,
                detected_studs=detected_studs
            )
            
            # Obtener el Top-15 de candidatos únicos
            candidates_ref = []
            if preds_first_pass:
                for cand in preds_first_pass[:15]:
                    ref_cand = cand["part_ref"]
                    if ref_cand not in candidates_ref:
                        candidates_ref.append(ref_cand)
            
            # Obtener la unión de colores válidos para estos candidatos
            for ref_cand in candidates_ref:
                allowed_color_names.update(efficientnet_clf.part_to_colors.get(ref_cand, set()))

        # Gating de Color (Pass 2): Calcular color gating de CIELAB directo
        if hierarchical_clf is not None:
            # Cenital:
            img_c_best = _get_img(best_cen_obs["frame_img_path"])
            bbox_cen_n_best = best_cen_obs["bbox_norm"]
            w_c_best, h_c_best = img_c_best.size
            px1_b = max(0, int(bbox_cen_n_best[0] * w_c_best)); py1_b = max(0, int(bbox_cen_n_best[1] * h_c_best))
            px2_b = min(w_c_best, int(bbox_cen_n_best[2] * w_c_best)); py2_b = min(h_c_best, int(bbox_cen_n_best[3] * h_c_best))
            mask_bin_best = _get_sam_mask(img_c_best, [px1_b, py1_b, px2_b, py2_b], (h_c_best, w_c_best))
            feat_cen_best = estimate_color_mlp_features(img_c_best, mask_bin_best, "cenital", ccm_params, is_simulation=is_simulation)
            
            p_cen_gated = None
            if feat_cen_best is not None:
                p_cen_gated = hierarchical_clf.predict_gated_probs_cielab(feat_cen_best, allowed_color_names, "cenital", is_simulation=is_simulation)
                
            # Lateral:
            p_lat_gated = None
            if USE_LATERAL_CAMERA and img_lat is not None and mask_lat is not None:
                feat_lat_best = estimate_color_mlp_features(img_lat, mask_lat, "lateral", ccm_params, is_simulation=is_simulation)
                if feat_lat_best is not None:
                    p_lat_gated = hierarchical_clf.predict_gated_probs_cielab(feat_lat_best, allowed_color_names, "lateral", is_simulation=is_simulation)
            
            # Resolver Color Fusionado Gated
            if p_cen_gated is not None and p_lat_gated is not None:
                p_combined_gated = p_cen_gated * p_lat_gated
                if np.sum(p_combined_gated) == 0:
                    p_combined_gated = p_cen_gated + p_lat_gated
            elif p_cen_gated is not None:
                p_combined_gated = p_cen_gated
            elif p_lat_gated is not None:
                p_combined_gated = p_lat_gated
            else:
                p_combined_gated = None
                
            # Asignar nombres finales resueltos
            if p_cen_gated is not None and len(p_cen_gated) > 0:
                top1_cen = np.argmax(p_cen_gated)
                color_name_cen = hierarchical_clf.classes[top1_cen]
                for c in CATALOG_COLORS:
            ...
            if p_lat_gated is not None and len(p_lat_gated) > 0:
                top1_lat = np.argmax(p_lat_gated)
                color_name_lat = hierarchical_clf.classes[top1_lat]
                for c in CATALOG_COLORS:
            ...
            if p_combined_gated is not None and len(p_combined_gated) > 0:
                top1_fused = np.argmax(p_combined_gated)
                color_name_fused = hierarchical_clf.classes[top1_fused]
                for c in CATALOG_COLORS:
                    if c["color_name"].strip().lower() == color_name_fused.strip().lower():
                        color_code_fused = c["color_code"]
                        color_hex_fused = c["color_hex"]
                        break
                        
        # Pass 3: Inferencia final de geometría utilizando el color fusionado extremadamente preciso
        detected_colors = [color_name_fused]
        with gpu_lock:
            preds = efficientnet_clf.classify(
                ...
                mask_lat=mask_lat,
                area_cenital=obs_apparent_area_mm2,
                detected_color=color_name_fused,
                ref_gt=ref_gt,
                measured_height=measured_height if not entry.get("is_simulation", False) else 0.0,
                measured_length=measured_length,
                measured_width=measured_width,
        ...
        if preds:
            ...
            if contour_matcher and should_use_contour:
                top_candidates = preds[:15]

                # Señal D: Alpha Dinámico — el peso del contorno aumenta cuando el KNN está empatado.
                # Si el KNN ya distingue claramente al ganador, el contorno solo confirma (alpha=0.50).
                # Si el KNN está empatado, el contorno es el árbitro final (alpha→0.80).
                if len(top_candidates) >= 2:
                    knn_delta = top_candidates[0]["score"] - top_candidates[1]["score"]
                    knn_confidence = min(1.0, knn_delta / 0.05)   # normalizado [0, 1]
                    alpha_contour = 0.50 + 0.30 * (1.0 - knn_confidence)  # [0.50, 0.80]
                else:
                    alpha_contour = 0.50
        ...
        result_dict = {
            ...
            "color_code_fused": color_code_fused,
            "color_name_fused": color_name_fused,
            "color_hex_fused": color_hex_fused,
