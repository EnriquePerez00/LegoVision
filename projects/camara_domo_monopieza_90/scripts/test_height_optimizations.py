...
def main():
    metadata_path = os.path.join(project_root, "data", "simulation_300", "simulation_metadata.json")
    ...
    # Cachar las alturas crudas en px de la última observación en cinta
    cached_data = []
    print("\n--- Extrayendo alturas en píxeles (caching) ---")
    for key, obs_list in piece_tracks.items():
        ...
        print(f"Ref {ref:5s} | GT={gt_height:5.2f}mm | RawHeightPx={raw_height_px:.1f}px")

    # Realizar grid-search de calibración
    print("\n--- Ejecutando Grid-Search de Calibración ---")
    best_err = float("inf")
    best_x_cam = 0.0
    best_px_per_mm = 0.0
    
    # Rango de X_camera de 230mm a 320mm
    x_cam_vals = np.linspace(230.0, 320.0, 91)
    # Rango de px_per_mm_lat de 9.5 a 11.5
    ...
    for x_cam in x_cam_vals:
        for px_mm in px_mm_vals:
            ...
            for item in cached_data:
                # Escalar px_per_mm por la resolución de la imagen (por si acaso w_l != 2048)
                px_per_mm_lat_scaled = px_mm * (item["w_l"] / 2048.0)
                raw_height_mm = item["raw_height_px"] / px_per_mm_lat_scaled
                
                # Perspectiva 3D con altura de la cámara a 20 mm
                D_center = np.sqrt((98.18 - x_cam)**2 + 20.0**2)
                D_piece = np.sqrt((item["x_belt_h"] - x_cam)**2 + item["y_belt_h"]**2 + 20.0**2)
    ...
    print("\n" + "="*60)
    print("RESULTADOS DE OPTIMIZACIÓN DE CALIBRACIÓN INTRÍNSECA (GRID-SEARCH)")
    print("="*60)
    print(f"Mejor Posición de Cámara X: {best_x_cam:.2f} mm")
    print(f"Mejor Escala Lateral:       {best_px_per_mm:.3f} px/mm")
    ...
    for item in cached_data:
        ...
        raw_height_mm = item["raw_height_px"] / px_per_mm_lat_scaled
        D_center = np.sqrt((98.18 - best_x_cam)**2 + 20.0**2)
        D_piece = np.sqrt((item["x_belt_h"] - best_x_cam)**2 + item["y_belt_h"]**2 + 20.0**2)
        pred_height = raw_height_mm * (D_piece / D_center)
        err = abs(pred_height - item["gt_height"]) / item["gt_height"] * 100.0
        print(f"Ref {item['ref']:5s} | GT={item['gt_height']:5.2f}mm | Pred={pred_height:5.2f}mm | Error={err:5.2f}%")
