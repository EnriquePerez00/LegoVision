...
def weighted_delta_e(lab1, lab2, w_L=0.1):
    dL = lab1[0] - lab2[0]
    da = lab1[1] - lab2[1]
    db = lab1[2] - lab2[2]
...
for c in hierarchical_clf.catalog_colors:
    if c["color_name"] in ["Dark Bluish Gray", "Dark Azure", "White", "Black", "Light Bluish Gray"]:
        ...
        dist_weighted = weighted_delta_e(lab_est, lab_ref, w_L=0.05)
        print(f"Catalog {c['color_name']:20s} | CIELAB Dist: {dist_ciede:.4f} | Weighted Dist (w_L=0.05): {dist_weighted:.4f}")
