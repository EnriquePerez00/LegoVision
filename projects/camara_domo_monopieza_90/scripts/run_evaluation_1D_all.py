...
# ── Pipeline Principal ──
def run_evaluation(metadata_path, report_path, use_dinov2_color=False, color_classifier_name="all_colors"):
    import os
    ...
    def process_entry(entry):
        ...
        # Promediar probabilidades acumuladas (Solo Cenital, omitiendo cámara lateral/frontal)
        p_combined = p_cen_sum / w_cen_sum if (w_cen_sum > 0 and p_cen_sum is not None) else None
        ...
        if hierarchical_clf is not None:
            ...
            # Resolver Color Fusionado Gated (Solo Cenital, omitiendo cámara lateral)
            ...
            if p_cen_gated is not None and len(p_cen_gated) > 0:
                ...
                if sample_idx < 5:
                    print(f"[DIAGNOSTIC] Sample {sample_idx}:")
                    print(f"  GT Color Code: {color_code_gt}")
                    print(f"  Cenital Pred: {color_name_cen} (prob: {p_cen_gated[top1_cen]:.4f})")
                    print(f"  Allowed Color Names: {list(allowed_color_names)}")
                for c in CATALOG_COLORS:
            ...
            # El color lateral y el fusionado heredan directamente del cenital
            color_name_lat = color_name_cen
            color_code_lat = color_code_cen
            color_hex_lat = color_hex_cen
            
            color_name_fused = color_name_cen
            color_code_fused = color_code_cen
            color_hex_fused = color_hex_cen
