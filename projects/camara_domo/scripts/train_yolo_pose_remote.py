...
def main():
    ...
    if is_kaggle:
        import glob
        cenital_matches = glob.glob("/kaggle/input/**/yolo_dataset_cenital", recursive=True)
        if not cenital_matches:
            raise FileNotFoundError("Could not find yolo_dataset_cenital inside /kaggle/input")
        dataset_cenital = cenital_matches[0]
        dataset_frontal = dataset_cenital.replace("yolo_dataset_cenital", "yolo_dataset_frontal")
        ...
        device = list(range(device_count)) if device_count > 1 else (0 if device_count == 1 else 'cpu')
        batch_size = 64 # VRAM saturada para dual T4
    else:
