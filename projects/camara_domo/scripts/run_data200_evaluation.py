# -*- coding: utf-8 -*-
"""projects/camara_domo/scripts/run_data200_evaluation.py
=========================================================
...
def main():
    metadata_path = os.path.join(project_root, "data", "data200", "simulation_metadata.json")
    report_json_path = os.path.join(project_root, "data", "reports", "data200_eval.json")
    output_html_path = os.path.join(project_root, "data", "reports", "data200_comparative_report.html")
    ...
    print("\n[Eval] Ejecutando run_evaluation.py sobre data200...")
    cmd_eval = [
    ...
    for idx, r in enumerate(eval_data["results"]):
        ...
        path_cen_img = os.path.join(project_root, "data", "data200", cen_file)
        path_lat_img = os.path.join(project_root, "data", "data200", lat_file)
        ...
        meas_height = r["lateral_height_meas_mm"]
        db_height = r.get("lateral_height_db_mm") or 0.0
        height_err = r.get("lateral_height_error_rel_pct", 0.0)

        badge_area_class = "badge-ok" if abs(area_err) < 15 else ("badge-neutral" if abs(area_err) < 30 else "badge-bad")
        badge_height_class = "badge-ok" if abs(height_err) < 15 else ("badge-neutral" if abs(height_err) < 30 else "badge-bad")
        ...
        <tr class="{row_class}">
            ...
            <td>
                ...
                <div style="font-size: 11px; margin-top: 4px;">
                    ...
                    <span>Altura GT: {db_height:.1f} mm</span>
                </div>
            </td>
            <td>
            ...
            <td>
                <div>{meas_height:.1f} mm</div>
                <div style="margin-top: 4px;"><span class="badge {badge_height_class}">{height_err:+.1f}%</span></div>
            </td>
            <td>
