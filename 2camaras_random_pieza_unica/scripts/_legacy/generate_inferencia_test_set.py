# -*- coding: utf-8 -*-
"""2camaras_random_pieza_unica/scripts/generate_inferencia_test_set.py
Set de inferencia de prueba: 10 muestras aleatorias + 2 forzadas (3023
pose 1 Trans-Brown / 3023 pose 1 Red), siguiendo el setup canonico
machine_vision (5500K, ring + 2 bars laterales, world strength 0.05).

Usado para validar el pipeline de inferencia con foco en la
...
Uso:
    /opt/homebrew/bin/blender -b -P \\
        2camaras_random_pieza_unica/scripts/generate_inferencia_test_set.py -- \\
            --num_random 10 \\
            --output_dir 2camaras_random_pieza_unica/data/inferencia_test \\
            --metadata_filename inferencia_test_metadata.json \\
            --seed 7

Las imagenes usan el prefijo "inftest_<idx:02d>_".
"""
...
def build_random_samples(combos, cache, num_random):
    """Genera num_random items aleatorios sobre el universo (ref,color,pose)."""
    universe = []
    for combo in combos:
        ref = combo["ref"]
        poses = cache.get(ref, [])
    ...
    n = len(universe)
    print(f"[plan] Universo (combo x pose): {n} items "
          f"(combos={len(combos)})")
    if not universe:
        return []
    if num_random <= n:
...
def build_forced_samples(cache):
    """Construye items para FORCED_SAMPLES."""
    out = []
    for spec in FORCED_SAMPLES:
        ref = spec["ref"]
        poses = cache.get(ref, [])
        if not poses:
            print(f"[plan][forced] WARN: {ref} sin poses en cache")
            continue
        pose = find_pose_by_index(poses, spec["pose_index"])
        if pose is None:
            print(f"[plan][forced] WARN: {ref} sin pose_index={spec['pose_index']} -> usar primera")
            pose = poses[0]
        cc = spec["color_code"]
...
def build_inferencia_plan(combos, cache, num_random):
    """Plan = num_random aleatorios + FORCED_SAMPLES."""
    rand = build_random_samples(combos, cache, num_random)
    forced = build_forced_samples(cache)
    plan = rand + forced
    print(f"[plan] {len(rand)} random + {len(forced)} forzadas = {len(plan)} muestras")
    return plan
...
def main():
    ...
    print(f"[infTest] Set {SET_ID}: {len(combos)} combinaciones (ref,color)")

    plan = build_inferencia_plan(combos, cache, parsed_args.num_random)
    print(f"[infTest] Plan: {len(plan)} muestras")
    ...
    for idx, item in enumerate(plan):
        ...
        # Persistir el ángulo Z efectivamente aplicado (para futuras
        # re-renderizaciones idempotentes desde el metadata).
        ...
        prefix = f"inftest_{idx:02d}_{mode_tag}_{ref}_{color_code}_p{pose_index}"
