# -*- coding: utf-8 -*-
"""2camaras_random_pieza_unica/scripts/generate_500_all_canonical_set.py
====================================================================
Genera N (default 500) muestras (cenital + lateral) de TODO el universo de sets
de la base de datos usando la ESCENA CANONICA.
"""
...
DEFAULT_OUTPUT_DIR = os.path.join(project_root, "data", "test500_all")
DEFAULT_METADATA_FILENAME = "random_500_metadata.json"
...
def main():
    ...
    if HAS_LOGGER:
        log_execution_header(
            log, "generate_500_all_canonical_set.py",
    ...
    log.info(f"[500set] Universo completo: {len(combos)} combinaciones (ref,color)")

    plan = build_sample_plan(combos, cache, pa.num_samples)
    log.info(f"[500set] Plan: {len(plan)} muestras")

    # Build escena canonica
    ...
    log.info("=" * 60)
    log.info(f"[500set DONE] {len(results_meta)} renders OK | "
