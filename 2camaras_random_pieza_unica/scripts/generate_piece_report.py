...
Usa renders existentes del set de posición aleatoria
(`data/random_position/random_position_metadata.json`).

Uso:
    .venv/bin/python 2camaras_random_pieza_unica/scripts/generate_piece_report.py --ref 3001
    .venv/bin/python 2camaras_random_pieza_unica/scripts/generate_piece_report.py --ref 3020 --pose 1
"""
...
# Orden de prioridad en sys.path:
#   1) project_root (módulo) → para `config_loader` / `config.yaml` propios.
#   2) legovic_root (raíz)   → para `database/` (única fuente de verdad —
#      BrickLink) e `inference/`. Como YA NO existen `database/` locales en
#      los módulos (ver backups/ tras la unificación), `from database.*` se
#      resuelve siempre contra `LegoVision/database/` aunque esté en 2ª pos.
...
def _hex_to_rgb(hex_str: str) -> list:
    """Convierte '#RRGGBB' (o 'RRGGBB') a [r, g, b] enteros 0-255."""
    h = hex_str.lstrip("#")
    if len(h) != 6:
        return [128, 128, 128]
...
def _rgb_to_lab(rgb_val) -> np.ndarray:
    """Conversión sRGB → CIELAB (D65) para comparación perceptiva."""
...
def _build_set_color_palette(set_id: str = "75078-1") -> list:
    """Construye la paleta de colores válidos para un set a partir de
    `database.set_catalog.REAL_SETS` (única fuente de verdad — BrickLink).

    Devuelve una lista de dicts únicos por `color_code` con la forma:
        {color_code, color_hex, color_name, rgb}
    """
    parts = REAL_SETS.get(set_id, {}).get("parts", [])
    seen = {}
    for p in parts:
        code = str(p.get("color_code", ""))
        if not code or code in seen:
            continue
        hex_str = p.get("color_hex", "#808080") or "#808080"
        seen[code] = {
            "color_code": code,
            "color_hex": hex_str,
            "color_name": p.get("color_name", "Unknown") or "Unknown",
            "rgb": _hex_to_rgb(hex_str),
        }
    return list(seen.values())


# Paleta cacheada del set actual (se construye una sola vez al importar).
_SET_COLOR_PALETTE = _build_set_color_palette("75078-1")


def find_nearest_set_color(crop_img: Image.Image) -> dict:
    """Encuentra el color de catálogo BrickLink más cercano (en Lab) a partir
    de la paleta real del set 75078-1, leída desde
    `database.set_catalog.REAL_SETS` (la única fuente de verdad).
    """
    palette = _SET_COLOR_PALETTE if _SET_COLOR_PALETTE else _build_set_color_palette("75078-1")
    fallback = palette[0] if palette else {
        "color_code": "85", "color_hex": "#6B6D67", "color_name": "Dark Bluish Gray"
    }
    fallback_clean = {k: v for k, v in fallback.items() if k != "rgb"}
    ...
    try:
        ...
        avg_lab = _rgb_to_lab(avg_rgb)

        best_match = palette[0]
        min_dist = float("inf")
        for sc in palette:
            sc_lab = _rgb_to_lab(sc["rgb"])
            d = float(np.linalg.norm(avg_lab - sc_lab))
            if d < min_dist:
                min_dist = d
                best_match = sc
        return {k: v for k, v in best_match.items() if k != "rgb"}
    except Exception as e:
...
<div class="img-label">Segmentación SAM Cenital</div>
</div>
<div>
...
<div>
<img src="data:image/png;base64,{img_crop_contour_lateral_b64}" alt="Contorno Lateral Crop">
<div class="img-label">Segmentación SAM Lateral</div>
</div>
</div>
</div>
...
Regla del pipeline de inferencia: si el color normalizado inferido por la cámara
cenital coincide con el de la cámara lateral, el color se da por <strong>bueno</strong>
y se usa para filtrar el conjunto de embeddings candidatos en la decisión final.
Si <strong>no</strong> coinciden, se marca como <em>error de color</em> y, en
ese caso, <strong>se usa el color cenital como decisión y se sigue filtrando</strong>
(por consigna explícita), pero el conflicto queda registrado.
</p>
<table>
...
<tr>
  <td><strong>Consenso Cenital ↔ Lateral (inferidos)</strong></td>
  <td colspan="2" class="{color_consensus_class}">{color_consensus_text}</td>
</tr>
<tr>
  <td><strong>Color usado para filtrado de embeddings</strong></td>
  <td colspan="2"><strong>{color_decision_used}</strong> &nbsp;—&nbsp; {color_decision_note}</td>
</tr>
</table>
...
def find_test_sample(part_ref: str, pose_index: int = None,
                     metadata_path: str = None,
                     sample_index: int = None) -> dict:
    """Find a test sample for the given part_ref in metadata.
    Si se da `sample_index`, se devuelve la entrada con ese `index` exacto
    (usado para casos del set 300 con varias muestras de la misma pieza+pose).
    """
    if metadata_path is None:
        metadata_path = os.path.join(
            project_root, "data", "random_position", "random_position_metadata.json"
        )
    if not os.path.exists(metadata_path):
        print(f"[ERROR] No test metadata found at {metadata_path}")
        return None
    with open(metadata_path, "r") as f:
        meta = json.load(f)
    renders = meta.get("renders", [])

    # Match exacto por sample_index (clave "index") si se solicita.
    if sample_index is not None:
        for entry in renders:
            if entry.get("index") == sample_index:
                if part_ref and entry.get("ref") != part_ref:
                    print(f"[WARN] sample_index={sample_index} corresponde a "
                          f"ref={entry.get('ref')} (no a {part_ref}). Se usa igualmente.")
                return entry
        print(f"[ERROR] No se encontró sample con index={sample_index}")
        return None
...
def generate_report(part_ref: str, pose_index: int = None,
    ...
    print(f"[Report] Generando report para pieza {part_ref} "
          f"(pose={pose_index}, sample_index={sample_index})...")
    ...
    actual_sample_idx = sample.get("index", sample_index)

    # 2. Load images
    cen_meta = sample["cameras"]["cenital"]
    lat_meta = sample["cameras"]["lateral"]
    def _resolve(meta_entry):
        p = meta_entry.get("image_path")
        if p and os.path.isfile(p):
            return p
        return os.path.join(test_dir, meta_entry["file_name"])
    img_cenital = Image.open(_resolve(cen_meta)).convert("RGB")
    img_lateral = Image.open(_resolve(lat_meta)).convert("RGB")
    ...
    # ─── Regla de consenso entre cámaras (igual que el pipeline de inferencia) ───
    # Si los códigos de color normalizados (cenital y lateral) coinciden:
    #   → consenso OK, color válido para filtrar embeddings (filtrado activo).
    # Si no coinciden:
    #   → ERROR de color. Por consigna, se sigue usando el color CENITAL como
    #     decisión de filtrado (pero queda registrado el conflicto).
    colors_agree = (nearest_cen["color_code"] == nearest_lat["color_code"])
    if colors_agree:
        color_consensus_class = "match"
        color_consensus_text = (
            f"✓ Consenso — cenital ({nearest_cen['color_code']}) == "
            f"lateral ({nearest_lat['color_code']})"
        )
        color_decision_used = nearest_cen["color_code"]
        color_decision_note = (
            "consenso entre cámaras → color usado como filtro de embeddings"
        )
    else:
        color_consensus_class = "mismatch"
        color_consensus_text = (
            f"✗ ERROR — cenital ({nearest_cen['color_code']}) ≠ "
            f"lateral ({nearest_lat['color_code']})"
        )
        color_decision_used = nearest_cen["color_code"]
        color_decision_note = (
            "<span class='mismatch'>conflicto entre cámaras</span> — "
            "se mantiene el color cenital como decisión de filtrado por consigna explícita"
        )
    ...
    html = HTML_TEMPLATE.format(
        ...
        img_crop_contour_lateral_b64=img_to_base64(get_sam_segmented_crop(crop_lat, segment_crop_sam(img_lateral, bbox_lat))),
        ...
        # ── Consenso cenital ↔ lateral (regla de decisión de filtrado) ──
        color_consensus_class=color_consensus_class,
        color_consensus_text=color_consensus_text,
        color_decision_used=color_decision_used,
        color_decision_note=color_decision_note,
    ...
    report_path = os.path.join(
        reports_dir, f"report_{part_ref}_pose{pose_idx:02d}{suffix}.html"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metadata", type=str, default=None,
                        help="Ruta al JSON de metadata (default: data/random_position/random_position_metadata.json)")
    parser.add_argument("--sample-index", type=int, default=None,
                        help="Si se especifica, busca el sample con ese 'index' exacto en el metadata "
                             "(útil para set 300 con varias muestras de la misma pieza/pose).")
    parser.add_argument("--out-suffix", type=str, default="",
                        help="Sufijo para el archivo HTML de salida (ej: 'sample116').")
