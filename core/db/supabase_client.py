...
def get_connection():
    """Obtiene una conexión a la base de datos o retorna una simulada si falla."""
    try:
        return psycopg2.connect(**DB_CONFIG, cursor_factory=psycopg2.extras.RealDictCursor)
    except psycopg2.OperationalError as e:
        print(f"\n[Supabase Client WARNING] No se pudo conectar a la base de datos ({e}). Se usará conexión de simulación.")
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT pose_index, contact_normal, face_class, contact_area, 
                           orientation_quat, orientation_euler, stability_ratio, tipping_energy_ratio
                    FROM stable_poses
