# -*- coding: utf-8 -*-
"""scripts/generate_validation_inference_renders.py
====================================================
Genera renders de validacion/inferencia para el pipeline.

Estrategia:
  - cfg.validation_renders.total_images imagenes totales (default 500)
  - Piezas aleatorias entre las 20 de cfg.pieces.selected_parts
  - 1 unica linea de piezas a lo ANCHO de la cinta (eje X)
  - Linea desplazada VAL_LINE_Y_BU (-0.5 BU = 5 cm) hacia la camara frontal
  - Misma camara, lente, resolucion y led_strip que el render frontal YOLO
  - Genera metadata.json con groundtruth para evaluacion end-to-end.

Salida:
  data/validation_renders/
    images/  val_00000.png ... val_00499.png
    metadata.json
"""
...
# Validation-specific parameters (cfg.validation_renders.*)
VAL_TOTAL_IMAGES = cfg.validation_renders.total_images
VAL_LINE_Y_BU = cfg.validation_renders.placement.line_y_bu       # -0.5 BU = 5 cm hacia camara frontal
VAL_MARGIN_BU = cfg.validation_renders.placement.margin_bu       # separacion entre piezas
VAL_LIMIT_X_BU = cfg.validation_renders.placement.limit_x_bu     # limite X (FOV)
VAL_OUTPUT_DIR = cfg.validation_renders.output_dir
VAL_IMAGE_PREFIX = cfg.validation_renders.image_prefix
...
def spawn_pieces_validation_line(tmap, geom_list, color_palette):
    """Spawnea una fila de piezas a lo ancho (eje X) en Y = VAL_LINE_Y_BU.

    Returns: lista de dicts {ref, color_hex, pose_index, rot_z_rad,
             position_bu, dimensions_mm} para metadata.json.
    """
    placed = []
    limit_bu = VAL_LIMIT_X_BU
    margin_bu = VAL_MARGIN_BU
    ...
    for part in selected_parts:
        ...
        # CLAVE: linea desplazada VAL_LINE_Y_BU desde el centro hacia la camara frontal
        oc.location = (cx, VAL_LINE_Y_BU, 0.0)
        bpy.context.view_layer.update()
        ...
        # Capturar dimensiones reales (BU -> mm: factor 10) para metadata.json
        bbox_final = [oc.matrix_world @ mathutils.Vector(c) for c in oc.bound_box]
        dim_x_mm = (max(pt.x for pt in bbox_final) - min(pt.x for pt in bbox_final)) * 10.0
        dim_y_mm = (max(pt.y for pt in bbox_final) - min(pt.y for pt in bbox_final)) * 10.0
        dim_z_mm = (max(pt.z for pt in bbox_final) - min(pt.z for pt in bbox_final)) * 10.0
        ...
        placed.append({
            ...
            "rot_z_rad": float(rot_z),
            "position_bu": [float(oc.location.x), float(oc.location.y), float(oc.location.z)],
            "dimensions_mm": [float(dim_x_mm), float(dim_y_mm), float(dim_z_mm)],
        })
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default=None,
                        help="Output dir (override cfg.validation_renders.output_dir)")
    parser.add_argument("--num_images", type=int, default=None,
                        help="N imagenes (override cfg.validation_renders.total_images)")
    args_raw = []
    if "--" in sys.argv:
        args_raw = sys.argv[sys.argv.index("--") + 1:]
    pa = parser.parse_known_args(args_raw)[0]

    # Resolver output_dir: por defecto VAL_OUTPUT_DIR (ej: "data/validation_renders")
    # Si es relativo, lo resolvemos contra el padre del setup (raiz del proyecto LegoVision)
    if pa.output_dir:
    ...
    if not os.path.isabs(output_dir):
        ...
        output_dir = os.path.join(parent_root, output_dir)
    images_dir = os.path.join(output_dir, "images")
    os.makedirs(images_dir, exist_ok=True)

    total_images = pa.num_images if pa.num_images is not None else VAL_TOTAL_IMAGES

    print(f"[ValGen] Output: {output_dir}")
    print(f"[ValGen] Total imgs: {total_images} | line_y_bu={VAL_LINE_Y_BU} | margin_bu={VAL_MARGIN_BU}")

    # Filtrar geometrias del set 75078-1 a las 20 piezas seleccionadas
    geom_list_full = load_set_geometries("75078-1")
    geom_list = [g for g in geom_list_full if g["ref"] in SELECTED_PARTS]
    print(f"[ValGen] Geometrias filtradas: {len(geom_list)}/{len(geom_list_full)}")
    ...
    metadata = {
        "config": {
            ...
            "line_y_bu": VAL_LINE_Y_BU,
            "margin_bu": VAL_MARGIN_BU,
            "limit_x_bu": VAL_LIMIT_X_BU,
            "camera": "frontal",
    ...
    frame_idx = 0
    while frame_idx < total_images:
        cleanup_placed_pieces()

        placed = spawn_pieces_validation_line(tmap, geom_list, color_palette)
        if not placed:
            continue
        ...
        img_fn = f"{VAL_IMAGE_PREFIX}_{frame_idx:05d}.png"
        scene.render.filepath = os.path.join(images_dir, img_fn)
        try:
            bpy.ops.render.render(write_still=True)
        except Exception as e:
            print(f"[WARN] Render fallido {frame_idx}: {e}")
            continue

        # Construir entrada de metadata para esta imagen
        img_meta = {
            "image": img_fn,
            "frame_idx": frame_idx,
            "num_pieces": len(placed),
            "pieces": [
                {
                    "ref": p["ref"],
                    "color_hex": p["color_hex"],
                    "pose_index": p["pose_index"],
                    "rot_z_rad": p["rot_z_rad"],
                    "position_bu": p["position_bu"],
        ...
        print(f"[OK] {img_fn} - {len(placed)} piezas (linea Y={VAL_LINE_Y_BU} BU)")
        frame_idx += 1
    ...
    print(f"[ValGen DONE] {frame_idx} imagenes en {output_dir}")
    print(f"[ValGen DONE] metadata.json escrito en {metadata_path}")
