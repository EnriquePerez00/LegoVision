...
KEYPOINTS_PATH = os.path.join(legovic_root, "2camaras_random_pieza_unica", "data", "canonical_keypoints.json")
...
def main():
    ...
    # Agrupar combos por referencia
    combos_by_ref = {}
    for combo in valid_combos:
        combos_by_ref.setdefault(combo["ref"], []).append(combo)
        
    all_refs = list(combos_by_ref.keys())
    sampled_instances = []
    for _ in range(pa.num_pieces):
        ref = random.choice(all_refs)
        # Seleccionar color aleatorio para esta geometría
        combo = random.choice(combos_by_ref[ref]).copy()
    ...
    # Inicializar Blender
    cam_cenital, cam_frontal = scene_canonical.build_scene_canonical(render_res=640, film_transparent=False)
