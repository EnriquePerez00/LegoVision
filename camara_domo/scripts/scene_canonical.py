...
def setup_camera(name, location):
    ...
    track.up_axis = "UP_Y"
    cam.data.type = "PERSP"
    if name == "Cam_Cenital" or "Cenital" in name or name == "Cam_Frontal" or "Frontal" in name:
        cam.data.lens = 55.0
    else:
...
def build_scene_canonical(render_res=640, film_transparent=False, enable_metal_gpu=True, configure_eevee=True):
    ...
    cam_frontal = setup_camera("Cam_Frontal", CAM_FRONTAL_LOC)
    setup_dome_light()
    if configure_eevee:
        try:
