...
def main():
    ...
    use_lateral = getattr(cfg.inference, "camara_lateral", False)
    print(f"[test50] Configured use_lateral={use_lateral}")

    cameras = {"cenital": cam_cenital}
    if use_lateral:
        cameras["lateral"] = cam_lateral
    results_meta = []
    skipped = []
    current_ref = None
    part_obj = None
    ...
    for idx, item in enumerate(samples):
        ...
        if ok and len(sample_meta["cameras"]) == len(cameras):
